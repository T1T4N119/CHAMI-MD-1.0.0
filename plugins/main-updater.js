...
cmd({
    ...
    try {
        await reply("🔍 Checking for 𝙲𝙷𝙰𝙼𝙸-𝙼𝙳 updates...");
        ...
        if (latestCommitHash === currentHash) {
            return reply("✅ Your 𝙲𝙷𝙰𝙼𝙸-𝙼𝙳 bot is already up-to-date!");
        }

        await reply("🚀 Updating 𝙲𝙷𝙰𝙼𝙸-𝙼𝙳 Bot...");

        // Download the latest code
        const zipPath = path.join(__dirname, "latest.zip");
        const { data: zipData } = await axios.get("https://github.com/T1T4N119/CHAMI-MD-1.0.0/archive/main.zip", { responseType: "arraybuffer" });
